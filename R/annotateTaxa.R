...
##'     point.
##' @param dataBaseDir The base directory of your blast
...
##' @param tempfasta A fasta file generated for the blast searche, a
##'     temporary file in the respective R temporary folder by
##'     default.
##' @param tempblast A blast tabular output file generated by the
...
getBlastTaxAnnot <- function (MA, dataBaseDir=Sys.getenv("BLASTDB"),
    ...
    if(!file.exists(tempblast)){
        message("STARTED running blast with command:\n",
            command)
        system(command)
        cat("\nFINISHED running blast\n")
    } else {
        message("file", tempblast, " exists, using existing file!", 
                 " To run blast again delete file")
    }
    ## we read that ouput 
    blast <- read.csv(tempblast, header=FALSE)
