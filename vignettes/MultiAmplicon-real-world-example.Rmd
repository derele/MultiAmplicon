...
```{r plottrack, message=FALSE, fig.show='hold'}
library(ggplot2)
plotPipelineSummary(tracking) +
  scale_y_log10() +
  geom_line()
...
annotation in one way or the other at some point, but here we can do
an export to a phyloseq object first. 

```{r handover, message=FALSE}
library(phyloseq)
...
We now can use phyloseq for one of the many supported analyses. Here..
