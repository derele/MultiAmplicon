...
We first
[download the data](https://svalbard.biologie.hu-berlin.de:443/d/c291b870178f4abc8c59/)
...
We start by sorting our amplicons by primer sequences cutting off the latter from sequencing reads.
...
errF <- learnErrors(unlist(getStratifiedFilesF(MA)), nread=1e6,
                    verbose=0)
errR <- learnErrors(unlist(getStratifiedFilesR(MA)), nread=1e6,
                    verbose=0)
...
MA <- removeChimeraMulti(MA, mc.cores=1)
MA <- fillSampleTables(MA)
```
