...
with error probabilities estimated separately for each amplicon (here
...
<!-- together <- lapply(MA6@sequenceTableNoChime, function (x) if(all(dim(x)>1))getSequences(x))[[3]] -->

<!-- alt <- lapply(MA.alt@sequenceTableNoChime, function (x) if(all(dim(x)>1))getSequences(x))[[3]] -->

<!-- table(alt%in%together) -->

<!-- alt.diff <- alt[!alt%in%together] -->
<!-- tog.diff <- together[!together%in%alt] -->
...
```
The above example would make sense if amplicons Amp1F.Amp1R and
Amp4F.Amp4R (1st and 4th in our object) had too little overlap
between forward and reverse reads to be merged and would better be
concatenated.
