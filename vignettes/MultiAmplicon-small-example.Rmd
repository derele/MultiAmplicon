...
So we can make a comparison between the two different versions:
...
```
So in this case we got nearly the same output: 215 of 8 (for the first
amplicon) and even higher proportions of identical speaks for error
...
have very few reads, it might make sense to pool them to improve error
estimates with larger size for the training set.
...
MA.mixed <- mergeMulti(MA3, justConcatenate=c(TRUE, FALSE, FALSE, TRUE, TRUE, TRUE))
```
...
own research it includes for now only a 18S annotation via blast. This
only works on 


```{r taxonAnnot, echo=TRUE}
...
```{r toPhloseq, echo=TRUE}
PHY <- toPhyloseq(MA7, samples=colnames(MA7))
